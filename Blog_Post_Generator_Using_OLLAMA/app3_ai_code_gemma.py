...
# """
def generateLLAMA_RESPONSE(input_text2):
    # """Construct model"""
    ...
    # *********************************
    llm1=Ollama(model="codegemma:2b")
    # """Construct prompt"""
    template_specific_usecase=f"Generate a python code in textual format with appropriate spaces & indentation to solve {input_text2} "
    prompt1=PromptTemplate(input_variables=["input_text2"],template=template_specific_usecase)
    ...
    x=llm1(prompt1.format(input_text2=input_text2))
    print(x)
    return x
original_title = '<p style="font-family:Jokerman; color: White; font-size: 35px;"><b> AI VIRTAUL CODE ASSISTANT </b></p>'
st.markdown(original_title, unsafe_allow_html=True)
# st.header("Blog Generation using LlAMA-2")

input_text = st.text_input("Enter the coding Problem to solve:")
...
output=st.button("Generate code")
if(output):
    output=(generateLLAMA_RESPONSE(input_text))
    output